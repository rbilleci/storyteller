...
This reference summarises the Black Sword Hack — Ultimate Chaos Edition System Reference Document (SRD) v1.0.2 for live adjudication. See ./attribution.md for the licence. Where this summary and the SRD disagree, the SRD governs.
...
Tools: `combat_begin_turn`, `combat_attack`, `combat_defend`, `combat_end_turn`.
...
Tools: `character_options` lists every origin, background, weapon table, and armour category; `character_create` rolls and writes the sheet. The source tables live in ./backgrounds.json.
...
A character advances after collecting Stories equal to their current level. Reaching level 2 needs 1 Story. Reaching level 3 needs 3 Stories in total. Levels 2 through 9 each add 1 hit point. Levels 2 and 6 raise one attribute; levels 4 and 8 raise two; an attribute raised by advancement stops at 18. Levels 3, 5, 7, and 9 grant a Gift, each Gift taken once. Level 10 upgrades the Doom die to d8 and adds no hit point.

Tool: `session_close` records Stories and reports eligibility. `character_advance` applies one level per call, gated by that eligibility: it takes the player's chosen attribute increases and Gift and writes them. It refuses during active combat.
...
Three parts are deterministic. Sorcery casting rolls the d100 spell table, and a Forbidden Knowledge character draws four spells at creation. A demonic invocation that depletes the Doom die rolls the six-face Demon's Revenge table. A sorcery critical failure rolls the six-face Torn Veil table. Each subsystem's individual power effects stay a narrator ruling: adjudicate them and record durable outcomes with `scene_commit`. Demon pacts, spirit alliances, and faerie ties each cap a character at two. See ../DEFERRED.md.
